...
- **Historical people** in the game, with the sources for each and the liberties taken: `docs/HISTORY.md`.
...
- **Audio**: every sound (footsteps, hooves, wheels, animals, voices and crowd murmur, bells, the hejnał, weapons,
  props, UI) is procedurally synthesised by `tools/gen_sfx.py` (numpy; ffmpeg/libvorbis for the Ogg files) into
  `assets/audio/`; no recordings or third-party samples are used. The generated audio is **CC BY 4.0**, the
  script MIT. See `docs/AUDIO.md`.
